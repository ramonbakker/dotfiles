...
do
    ...
    CURRENT_DAY_ENTRY_DIFF=`/usr/local/bin/datediff -i '%Y%m%dT%H%M%SZ' -f '%S' $TIMEW_DAY_START $TIMEW_DAY_END`
    TIMEW_DAY_TOTAL=$(($TIMEW_DAY_TOTAL + $CURRENT_DAY_ENTRY_DIFF))

    CURRENT_DAY_ENTRY_TAG=`echo $TIMEW_EXPORT | jq --argjson day_entry_number $DAY_ENTRY_NUMBER '.[$day_entry_number].tags[0]'`
...
echo "$TIMEW_DAY_HOURS:$TIMEW_DAY_MINUTES:$TIMEW_DAY_SECONDS [$TIMEW_CURRENT_HOURS:$TIMEW_CURRENT_MINUTES:$TIMEW_CURRENT_SECONDS $CURRENT_DAY_ENTRY_TAG]"
