...
if [ "$DAY_ENTRY_NUMBER" == 0 ]
then
    CURRENT_DAY_ENTRY_STATUS_LINE="-"
else
    CURRENT_DAY_ENTRY_STATUS_LINE="$STATUS_INDICATOR $TIMEW_CURRENT_HOURS:$TIMEW_CURRENT_MINUTES:$TIMEW_CURRENT_SECONDS $CURRENT_DAY_ENTRY_TAG"
fi

echo "$TIMEW_DAY_HOURS:$TIMEW_DAY_MINUTES:$TIMEW_DAY_SECONDS [$CURRENT_DAY_ENTRY_STATUS_LINE]"
